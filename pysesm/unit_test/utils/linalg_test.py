...
def test_to_triu_matrix():
    ...
    expected_matrix = torch.tensor([[1, 2, 3], [0, 4, 5], [0, 0, 6]], dtype=torch.float32)

    result_matrix = to_triu_matrix(non_zeros)
    assert torch.equal(result_matrix, expected_matrix), "Upper triangular matrix conversion failed"
    ...
    assert torch.equal(result_matrix_empty, expected_matrix_empty), "Empty vector conversion failed"
...
def test_gram_schmidt():
    ...
    expected_Q = torch.tensor([[math.sqrt(2)/2, math.sqrt(2)/2], [math.sqrt(2)/2, -math.sqrt(2)/2]], dtype=torch.float32)
    ...
    assert torch.allclose(result_Q_ortho, Q_ortho, atol=1e-5), "Orthonormalization of orthonormal matrix failed"
...
def test_generate_random_vectors():
    ...
    assert random_vectors.shape == (features, features), "Generated vectors have incorrect shape"

    # Ensure values are within the specified range
    assert (random_vectors >= min_val).all() and (random_vectors <= max_val).all(), "Values are out of the specified range"
...
def test_get_upper_triangle():
    ...
    assert torch.equal(result_upper, expected_upper), "Upper triangular extraction failed"
    ...
    assert torch.equal(result_upper_non_square, expected_upper_non_square), "Upper triangular extraction for non-square matrix failed"
...
def test_reshape_upper_triangle():
    ...
    assert torch.equal(result_reshaped, expected_reshaped), "Reshape upper triangle failed"

    # Test with padding requirement
    upper_triangle_padding = torch.tensor([1, 2, 3, 4], dtype=torch.float32)
    expected_reshaped_padding = torch.tensor([[1, 2, 3], [4, 0, 0]], dtype=torch.float32)

    result_reshaped_padding = reshape_upper_triangle(upper_triangle_padding, n)
    assert torch.equal(result_reshaped_padding, expected_reshaped_padding), "Reshape upper triangle with padding failed"
    ...
    assert torch.equal(result_reshaped_empty, expected_reshaped_empty), "Reshape upper triangle with zero elements failed"
