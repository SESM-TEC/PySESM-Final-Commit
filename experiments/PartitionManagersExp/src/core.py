...
def train_one_run(cfg, logger, func_obj):  # pylint: disable=too-many-locals, invalid-name
    """Run a single training and evaluation using configuration ``cfg``.

    Args:
        cfg: Hydra configuration object with experiment parameters.
        logger: Logger instance to use for progress messages.
        func_obj: Callable objective function used to generate synthetic data.

    Returns:
        dict with evaluation metrics and metadata (MSE, MAE, Train_Time, ...).
    """
    ...
    # Preparamos y_test_norm para pasarlo al modelo
