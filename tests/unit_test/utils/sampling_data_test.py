...
def test_generate_uniform_sampling():
    ...
    assert np.array_equal(sampled_indices, expected_indices), "Sampled indices are not reproducible"

    # Ensure the indices are unique
    assert len(set(sampled_indices)) == n_samples, "Sampled indices are not unique"

    # Ensure indices are within valid range
    assert all(0 <= idx < total_points for idx in sampled_indices), "Sampled indices are out of range"

    # Test edge cases
    sampled_indices_edge = generate_uniform_sampling(total_points, SEED, total_points)
    assert len(sampled_indices_edge) == total_points, "Edge case failed for sampling all points"
    
    # Check with min_separation (to be implemented)
    sampled_indices_separated = generate_uniform_sampling(total_points, SEED, n_samples, min_separation=2)
    assert len(sampled_indices_separated) == n_samples, "Number of samples with separation is incorrect"
...
def test_sample_data():
    ...
    expected_X = torch.tensor([[0, 1000], [10, 1010], [50, 1050], [100, 1100], [150, 1150]], dtype=torch.float32)
    ...
    expected_X_diff = torch.tensor([[999, 1999], [500, 1500], [250, 1250], [750, 1750]], dtype=torch.float32)
    expected_y_diff = torch.tensor([2999, 2500, 2250, 2750], dtype=torch.float32)

    assert torch.allclose(X_diff, expected_X_diff, atol=1e-5), "Sampled X data with different indices is incorrect"
    assert torch.allclose(y_diff, expected_y_diff, atol=1e-5), "Sampled y data with different indices is incorrect"
