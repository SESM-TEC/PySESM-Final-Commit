...
*   **Partition Managers (`pysesm.blocks`):** This component is responsible for dividing the input space.
    ...
    *   `AdaptivePartitionManager`: Uses a KD-Tree to dynamically partition the space based on data density, creating blocks of varying sizes where needed.
...
Multi-block partitioning divides the input domain into several smaller regions (blocks) so the model can learn local behaviour in each block independently while sharing a global dictionary. This approach improves scalability and accuracy on complex, non-stationary functions because each block fits simpler local patterns. Use `UniformPartitionConfig` to create regular grids (or `AdaptivePartitionConfig` for data-driven splits), and tune `T` (blocks per dimension) and `overlap_ratio` to balance locality versus continuity between blocks.
...
### Adaptive (KD-Tree) Partitioning example `examples/bsesm_example.py`

While `UniformPartitionConfig` splits the domain into a regular grid, the
`AdaptivePartitionManager` divides the space using a **KD-Tree** that adapts to
the data distribution. Instead of fixed-size cells, it produces blocks of
**varying size**: finer where samples are dense and coarser where they are
sparse. This is useful for non-stationary functions whose interesting behaviour
is concentrated in particular regions of the input space.

To use it, build a `KDTreeStrategyConfig` (which describes how the tree splits)
and pass it to an `AdaptivePartitionConfig`. The only change required is in the
partition configuration block:

```python
# From bsesm_example.py
...
| Parameter | Belongs to | Description |
|-----------|-----------|-------------|
| `maxNodeSize` | `KDTreeStrategyConfig` | Maximum number of points a KD-tree node may hold before it is split. Smaller values produce more (finer) blocks. |
| `data_wrapper` | `KDTreeStrategyConfig` | Class used to encapsulate the data stored in each node (typically `SESMData`). |
| `partition_strategy` | `AdaptivePartitionConfig` | Strategy constructor used to build the partition (here `KDTreeStrategy`). |
| `strategy_config` | `AdaptivePartitionConfig` | The strategy configuration instance (the `KDTreeStrategyConfig` above). |
| `overlap_ratio` | `AdaptivePartitionConfig` | Fractional overlap between adjacent blocks, for smooth transitions. |

The rest of the training pipeline (dictionary, sparse coding, model fitting)
remains identical to the uniform case. The figure below shows the resulting
2D partition: unlike the regular grid, the KD-Tree produces blocks of varying
size that adapt to the data density.

<p align="center">
  <img src="figs/kdtree_bsesm_example.png" width="700" />
</p>

Training the model with the `VisualizerHook` captures the evolution of the
dictionary within each KD-Tree block:

<p align="center">
  <img src="figs/bsesm_evolution_kdtree.gif" width="500" />
</p>

For a complete, runnable 2D example, see `examples/bsesm_example.py`.
...
*   `pysesm.blocks.KDTreeStrategy`: KD-Tree partition strategy used by the adaptive partition manager.
*   `pysesm.blocks.KDTreeStrategyConfig`: Configuration for the KD-Tree partition strategy (`maxNodeSize`, `data_wrapper`).
